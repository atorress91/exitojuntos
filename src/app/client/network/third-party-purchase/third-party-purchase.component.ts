...
export class ThirdPartyPurchaseComponent implements OnInit, OnDestroy {
  ...
  getStepTitle(): string {
    ...
    const titles = [
      ...
      this.selectedFlow === 'purchase'
        ? 'THIRD_PARTY_PURCHASE.SELECT_exitojuntosS'
  ...
  isSelected(exitojuntos: Product): boolean {
    return this.selectedItems.some(item => item.id === exitojuntos.id);
  }
