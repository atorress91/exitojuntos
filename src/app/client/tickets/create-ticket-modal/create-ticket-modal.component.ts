...
export class CreateTicketModalComponent implements OnInit {
  ...
  createTicket(): void {
    ...
    this.ticket.categoryId = Number.parseInt(
  ...
  startTicketImageUpload(): void {
    ...
    const uploads = this.files.map(file => this.uploadFile(file, filePathBase));
  ...
  private uploadFile(
    file: File,
    filePathBase: string,
  ): Observable<TicketImagesRequest> {
    ...
    return new Observable<TicketImagesRequest>(subscriber => {
      uploadTask.on(
        ...
        () => this.handleUploadComplete(uploadTask, subscriber),
      );
    });
  }

  private handleUploadComplete(uploadTask: any, subscriber: any): void {
    getDownloadURL(uploadTask.snapshot.ref).then(
      downloadURL => {
        const imageRequest = new TicketImagesRequest();
